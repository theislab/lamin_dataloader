...
def mock_anndata():
    ...
    var = pd.DataFrame({"gene_symbol": var_names, "feature_type": ["Gene Expression"] * n_vars})

    # Create observation metadata
    obs = pd.DataFrame({"cell_type": ["T_cell"] * n_obs, "batch": ["batch1"] * n_obs})
...
class MockCollection(Collection):
    ...
    def __getitem__(self, idx):
        if idx < self.n_obs_list[0]:
            return {"dataset": 0, "X": np.array([1.0, 2.0, 3.0])}
        else:
            return {"dataset": 1, "X": np.array([4.0, 5.0, 6.0])}
...
def test_tokenized_dataset_initialization(mock_collection):
    ...
    dataset = TokenizedDataset(collection=mock_collection, tokenizer=tokenizer, obs_keys=[], normalization="log1p")
